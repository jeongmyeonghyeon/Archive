...
class Node(object):
    ...
    HTML태그 하나를 가지는 클래스
        ...
        가장 큰 범위는 <html></html>
    """
    # 원하는 태그 영역 내의 모든 태그를 가져오는 패턴, pattern_tag_base 변수에 할당

    # <{tag}.*?>
    # 조건: 전달받은 태그명에 속성이 지정되어 있을 경우(클래스명 또는 아이디명이 지정되어 있는 경우)에도 찾아야 한다.
    # '<전달받은 태그명' 으로 시작하고
    # ' . (하나의 문자를) * (앞의 항목을 0회 이상 찾는다/없어도 상관없음)' == 단일 문자를 포함하는 패턴을
    # ' ? (0회 또는 1회 검색 == 없거나 있거나 1번만 검색)` 한다. > 를 만나기 전까지.

    # \s*([.\w\W]*?)\s*
    # 조건: 태그영역 내의 모든 문자열(탭, 줄바꿈, 개행문자 포함)가 지정되어야 한다.
    # '\s*' (white space character_탭/줄바꿈/개행문자, 없어도 상관없음)
    # [. (단일문자) 또는 \w (문자) 또는 \W (비문자)] 를 * (앞의 항목을 0회 이상 찾는다/없어도 상관없음) == 모든 문자열을 포함하는 패턴(태그 영역내의 모든 부분)
    # () .group()를 통해 추출할 때, 문자만 추출하기 위해 그룹화
